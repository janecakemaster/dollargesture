...
var DPMainToolbar = new Class({
  ...
  initialize: function() {
    ...
    this.mItems = [
        ...
        new DPMainToolbar_Widget_Button({
          ...
          'onRefresh': function() {
            ...
            if(locked_states.length > 1 || (1 == locked_states.length && 0 == locked_states.pop())) {
              this.mEl.removeClass('inactive');
            }
            else {
              this.mEl.addClass('inactive');
            }
          },
      ...
      new DPMainToolbar_Widget_ButtonGroup('', [
...
//      }),
      new DPMainToolbar_Widget_ButtonGroup('action', [
        ...
        new DPMainToolbar_Widget_Button({
          ...
          'onClick': function(pEl) {
            var version = new DPProject_VersionCommit({
              'container': pEl.getParent(),
              'id_project': dpManager.dpProject.getIdProject(),
              'onCommit': function() {
                pEl.addClass('inactive');
                GAnal.trackEvent('editor', 'create-iteration');
                mixpanel.track('Create iteration');
              },
              'onDispose': function() {
                pEl.removeClass('inactive');
              }
            });

            if(!pEl.hasClass('inactive')) {
              version.commit();
              version.getEl().position({
                'relativeTo': dpManager.dpWorkBench.getWorkBenchElMain(),
                'position': 'upperRight',
                'edge': 'upperRight',
                'offset': {
                  'y': 10 + dpManager.dpWorkBench.getWorkBenchElMain().getScrollTop(),
                  'x': -27 + dpManager.dpWorkBench.getWorkBenchElMain().getScrollLeft()
                }
              });
            }
          }
    ...
    return new DPMainToolbar_Widget_Button({
      ...
      'onClick': function(pEl) {
        ...
        var share_options = {
          ...
          'preview_password': dpManager.dpProject.mData.project_password,
        };
...
var DPMainToolbar_Widget_Button = new Class({
  ...
  render: function(pContainerEl) {
    ...
    this.mTextEl = new Element('i').inject(this.mEl);
    this.setText();
    
    this.mTooltipEl = this.mEl.getElement('span.hint em');
  ...
  renderTooltip: function() {
    this.mTooltipEl = new Element('em');
...
var DPWorkBench_ResponsiveWidget = new Class({
  ...
  render: function(pEl) {
    ...
    var option_el_helper = function(pEl) {
      var for_id = pEl.get('id');
      var pLabel = pEl.getAttribute('data-text');
      return ['div.options', [
        pEl,
        ['label[for=%s]'.sprintf(for_id), pLabel]
      ]]
    }
    var size_pickers = [];
    var options = [];
    ...
    dpManager.dpProject.mVersions.each(function(pSize) {
      ...
      var elem = new Element('input', {
        ...
        'events' : {
          'change': function() {
            self.toggleVersion(pSize, this.get('checked'))
          }
        }
      });
      ...
      size_pickers.push(option_el_helper(this['resp'+pSize.version]))
    }.bind(this));

    this.mEl = new Element('div', {
      'class': 'workbench-options'
    })
      .inject(pEl)
      .microjungle([size_pickers]);

    /*
    Select do ktorego wrzucone beda opcje
    */
    this.mainSelect = new Element('select', {
      'id': 'main-select',
      'title': 'When creating new breakpoint, elements will be copied from this view'
    }).inject(pEl)
      .microjungle(options)
      .addEvent('change', this.changeMainVersion);

    /*
    Div zawierajacy select i label
    */
    this.selectDiv = new Element('div', {
      'class': 'workbench-options copy'
    }).inject(pEl)
      .microjungle([
        ['div.options',
          [['label',{'for':'main-select', 'title':'When creating new breakpoint, elements will be copied from this view'}, 'When creating new breakpoint, copy all elements from:'], this.mainSelect]
        ]
      ]);
  ...
  changeMainVersion: function() {
    dpManager.dpProject.setMainVersion(this.get('value'));
  },

  toggleVersion: function(pSize, pCheck) {
    var version = pSize.version;
    if(!pCheck) {
      this.mToolbar.destroy();
      DSWindow.CreateConfirm(transtext('Are you sure you want to delete '+ pSize.text +'? <br/><br/>Please note that the '+ pSize.text +' breakpoint will be removed from all pages in this wireframe and that you will not be able to undo this action.'), transtext('Delete '+ pSize.text +'? '), transtext('Yes'), transtext('No'), function() {
...
var DPMainToolbar_Widget_Button_Responsive = new Class({
  ...
  options: {
    'class_name': 'responsive inactive',
    'text': 'Responsive',
  ...
  showToolbar: function(pEl) {
    ...
    this.mToolbar.addWidget(new dmsDPPropertyToolbar_Widget_Separator({
...
var DPMainToolbar_Widget = new Class({
  ...
  refreshMinMax: function(pActiveEl, pMin, pMax, pProps) {
    (function() {
      ...
      if(undefined !== pMin && undefined !== pMax) {
...
var DPSidebar = new Class({
  ...
  initialize: function(pOptions) {
    ...
    var new_design_cookie = new Cookie('new_design_info', {
      'duration': 999,
      'path': '/'
    })
    
    // Jeśli wogóle nie widział wcześniej nowego designu
    if(!new_design_cookie.read()) {
      this.mItems.news.mEl.fireEvent('click');
      new_design_cookie.write(1)
    }
    // Jesli wszedł pierwszy raz po zaimportowaniu z uxporn
//    else if(dpManager.isUxpornFirstUsage()) {
//      this.mItems.news.mEl.fireEvent('click', ['uxporn_mode']);
//    }
  },
    ...
    this.mItems.each(function(pItem, pKey) {
      var container_el = 'nav' === pItem.getSection() ? nav_el : subnav_el,
          list_el = new Element('li', {
              'class': pKey + (pItem.options.custom_class ? ' '+pItem.options.custom_class : '')
            })
            .inject(container_el);
...
var dmsDPPreview_Menu = new Class({
  ...
  load: function(pData) {
    ...
    var load_menu_page = function(pData) {
      ...
      this.loadMenuPage(pData.id_page)
    }.bind(this);
      ...
      if(!pItem.version_of) {
        ...
        node = parent_node.insert({
          ...
          'onSelect': function() {
            ...
            if(this.data && this.data.id_page) {
        ...
        if (Preview.getData().id_page == pItem.id_page || open_nodes.contains(pItem.id_page)) {
          node.toggleParent();
        }
        }
      }
