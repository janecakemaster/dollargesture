...
var DPComet = new Class({
  ...
  */
  ...
  reconnect: function() {
    ...
    var reconnect_end = function() {
      // zapisz niewyslane zmiany lokalne
      if(changes === false) {
    ...
    var comet = new Request.Comet({
      // 'url': '/ajax/dmsDPComet/Push/',
      'onPush': reconnect_changes,
      'onEnd': reconnect_end,
      'onFail': this.onTimeout.bind(this),
      'onTimeout': this.onTimeout.bind(this)
    });
    
    comet.send({'id_project': this.mIdProject, 'project_hash': this.mProjectHash, 'thread_id': this.mThreadID, 'script_time': 1});
  },
...
var DPPageComments_Pin_Popup = new Class({
  ...
  save: function(pMode) {
    ...
    mixpanel.track('Add comment response');
  },
...
var DPPageComments_Pin = new Class({
  ...
  update: function() {
    ...
    this.mEl.setStyles({
      'left': this.mCommentRef.get('pos_x').toInt(),
      'top': this.mCommentRef.get('pos_y').toInt()
    });
